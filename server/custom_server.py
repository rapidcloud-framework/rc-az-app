...
def get_app_service_plans():
    subscription_id = os.environ["AZURE_SUBSCRIPTION_ID"]
    credential = DefaultAzureCredential()
    service_plan_client = WebSiteManagementClient(credential, subscription_id)
    entities = service_plan_client.app_service_plans.list()
    plans = []

    for plan in list(entities):
        output_dict = {}
        label = f"{plan.name}"
        output_dict['value'] = {}
        output_dict['type'] = "Theia::Option"
        output_dict['label'] = label
        output_dict['value']['type'] = "Theia::DataOption"
        output_dict['value']['value'] = plan.id
        plans.append(output_dict)

    return plans

def get_app_service_envs():
    subscription_id = os.environ["AZURE_SUBSCRIPTION_ID"]
    credential = DefaultAzureCredential()
    web_client = WebSiteManagementClient(credential, subscription_id)
    entities = web_client.app_service_environments.list()
    envs = []

    for env in list(entities):
        output_dict = {}
        label = f"{env.name}"
        output_dict['value'] = {}
        output_dict['type'] = "Theia::Option"
        output_dict['label'] = label
        output_dict['value']['type'] = "Theia::DataOption"
        output_dict['value']['value'] = env.id
        envs.append(output_dict)
...
def custom_endpoint(action, params, boto3_session, user_session):
    ...
    elif action == "service_plans":
        return get_app_service_plans()
    elif action == "app_service_envs":
        return get_app_service_envs()
